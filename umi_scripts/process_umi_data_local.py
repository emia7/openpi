...
def quaternion_to_euler(quat):
    ...
    # scipy expects [x, y, z, w], which matches our data format
    r = R.from_quat(quat)
    return r.as_euler('xyz')


# === [新增] 可视化函数：只画状态 (Pose + Clamp) ===
def plot_states(pose_data, clamp_data, output_path):
    """
    Plot position, rotation (euler), and clamp data aligned to image frames.
    
    Args:
        pose_data: (T, 7) array [x, y, z, qx, qy, qz, qw]
        clamp_data: (T,) array
    ...
    # Unwrap angles (解决 0/2pi 跳变问题，让曲线连续)
    for i in range(3):
    ...
    for i in range(3):
        ...
        ax.plot(steps, pos[:, i], 'b-', linewidth=1.5, label='Pos')
    ...
    for i in range(3):
        ...
        ax.plot(steps, euler[:, i], 'g-', linewidth=1.5, label='Rot')
        ax.set_title(f'Rotation {rot_labels[i]}')
        ax.set_xlabel('Frame')
        ax.grid(True, alpha=0.3)
        
    # Plot Clamp
    ax = fig.add_subplot(gs[2, :])  # Span all columns
...
def main(bag: str, serial: str, out_dir: str, data_idx:int):
    ...
    with rosbag.Bag(str(bag), "r") as bag_obj:
        for topic, msg, t in bag_obj.read_messages(topics=[img_topic, pose_topic, clamp_topic]):
            ...
            if topic == img_topic:
                ...
                try:
                    ...
                    # Fallback if message type is different than expected simple format
                    pass

    print(f"  Loaded {len(images)} images, {len(poses)} poses, {len(clamps)} clamp msgs.")
    ...
    if len(poses) > 0:
        ...
        print("Warning: No pose data found! Filling with zeros.")
        aligned_pose = np.zeros((len(images), 7))
        aligned_pose[:, 6] = 1.0 # identity quaternion w=1
    ...
    if len(clamps) > 0:
        ...
        print("Warning: No clamp data found! Filling with zeros.")
        aligned_clamp = np.zeros(len(images))
    ...
    # === [新增] 生成可视化图表 ===
    plot_path = out_dir / f"episode{data_idx}_states.png"
