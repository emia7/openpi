...
class FrankaRelInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        for i in range(horizon):
            ...
            # pose6d转成rot6d
            rel_pose_mat = pose6_to_mat(rel_pose_6d)
            rel_pose_rot6d = mat_to_pose10d(rel_pose_mat)

            # 组合
            rel_action = np.concatenate([rel_pose_rot6d, [target_gripper_action]])
        ...
        if current_dim < target_dim:
            ...
            inputs["actions"] = np.concatenate([processed_actions, padding], axis=-1)
        else:
...
class FrankaRelOutputs(transforms.DataTransformFn):
    def __call__(self, data: dict) -> dict:
        ...
        # 拼接
        action_7d = np.concatenate([pose_6d, gripper], axis=-1)
