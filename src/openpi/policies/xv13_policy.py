...
class XVInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # --------------------------
        head_img = data.get("head_view", None)
        left_img = data.get("left_view", None)
        head_img = _parse_image(head_img)
...
class XVOutputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        print(data["state"].keys())
        cur_pos = np.asarray(data["eef_pos"], dtype=np.float32)      # (3,)
        cur_rotvec = np.asarray(data["eef_rot_axis_angle"], dtype=np.float32)  # (3,)
        gripper_raw = np.asarray(data["gripper_width"], dtype=np.float32)  # (1,)

        

        cur_pose_mat = pose6_to_mat(np.concatenate([cur_pos, cur_rotvec], axis=-1))  # (4,4)
        action_mat = pose10d_to_mat(act[..., :10])  # (H,4,4)
        base_action_mat = convert_pose_mat_rep(
            action_mat,
            cur_pose_mat,
            pose_rep='relative',
            backward=True,
        ...
        dgripper = gripper - gripper_raw  # (H,1)

        R_cur = cur_pose_mat[:3, :3]
        t_cur = cur_pose_mat[:3, 3]

        R_tgt = base_action_mat[..., :3, :3]
        t_tgt = base_action_mat[..., :3, 3]

        # base-frame translational delta
        dpos_base = t_tgt - t_cur  # (H,3)

        # base-frame rotational delta: R_tgt * R_cur^{-1}
        dR = np.matmul(R_tgt, R_cur.T)  # (H,3,3)
        drot_base = R.from_matrix(dR).as_rotvec()  # (H,3)
      
        action_7d = np.concatenate([dpos_base, drot_base, dgripper], axis=-1).astype(np.float32)
        '''
      ...
      # action_7d = np.array([0.01 ,0,0, 0, 0, 0, 0], dtype=np.float32)
      # action_7d =np.repeat(action_7d[None, :], 16, axis=0)
      # base = np.array([1, 0, 0,0,0, 0, 0], dtype=np.float32) * 0.01
      # increments = np.arange(16, dtype=np.float32)[:, None] * base
      # action_7d = increments + base

      # action_7d = np.array([0,0.01,0,0,0, 0,0 ], dtype=np.float32)
      # action_7d =np.repeat(action_7d[None, :],16, axis=0)
