...
import openpi.policies.dual_franka_absstate_basedelta_policy as dual_franka_absstate_basedelta_policy
...
class LeRobotXVDualDataConfig(DataConfigFactory):
    ...
    State encodes: hands_rel_xyz (3) + hands_rel_rot6d (6) = 9D
    ...
    Expected dataset keys (per frame):
      ...
      - hands_rel_xyz (3), hands_rel_rot6d (6): Inter-gripper relative pose (right in left frame)
...
_DUAL_FRANKA_IMG_LEFT_LUMOS = "observation.images.left_wrist_lumos"
_DUAL_FRANKA_IMG_RIGHT_LUMOS = "observation.images.right_wrist_lumos"
_DUAL_FRANKA_IMG_LEFT_RS = "observation.images.left_wrist_d435"
_DUAL_FRANKA_IMG_RIGHT_RS = "observation.images.right_wrist_d435"
_DUAL_FRANKA_IMG_THIRD_DEFAULT = "observation.images.third_d455"
_DUAL_FRANKA_ROOT = "/mnt/public1/chenshuaiwen/.cache/hf_home/dual_franka"


def _dual_franka_inputs_class(variant: DualFrankaVariant) -> type:
    if variant == "mask_lumos":
        return dual_franka_policy.DualFrankaMaskThirdLumosWristInputs
    if variant == "mask_rs":
        return dual_franka_policy.DualFrankaMaskThirdRealSenseWristInputs
    if variant == "high_lumos":
        return dual_franka_policy.DualFrankaHighThirdLumosWristInputs
    if variant == "high_rs":
        return dual_franka_policy.DualFrankaHighThirdRealSenseWristInputs
    if variant == "absdelta_mask_lumos":
        return dual_franka_absstate_basedelta_policy.DualFrankaAbsStateBaseDeltaMaskLumosInputs
    if variant == "absdelta_mask_rs":
        return dual_franka_absstate_basedelta_policy.DualFrankaAbsStateBaseDeltaMaskRSInputs
    if variant == "absdelta_high_lumos":
        return dual_franka_absstate_basedelta_policy.DualFrankaAbsStateBaseDeltaHighLumosInputs
    if variant == "absdelta_high_rs":
        return dual_franka_absstate_basedelta_policy.DualFrankaAbsStateBaseDeltaHighRSInputs
    raise AssertionError(variant)


def _dual_franka_outputs_class(variant: DualFrankaVariant) -> type:
    if variant.startswith("absdelta_"):
        return dual_franka_absstate_basedelta_policy.DualFrankaAbsStateBaseDeltaOutputs
    return dual_franka_policy.DualFrankaDualHandOutputs


@dataclasses.dataclass(frozen=True)
class LeRobotDualFrankaDataConfig(DataConfigFactory):
    """
    Dual-arm Franka LeRobot export (``dual_scripts/convert_dual_franka_data_to_lerobot.py``).

    Four setups only: third view **high** vs **masked**, and wrists **Lumos** vs **RealSense**.
    Multiple tasks (e.g. handover + reverse) and episode counts are **not** separate TrainConfigs:
    Train configs use a fixed ``repo_id`` matching the LeRobot export path.
    """

    variant: DualFrankaVariant = "high_lumos"
    third_image_key: str = _DUAL_FRANKA_IMG_THIRD_DEFAULT

    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        if self.variant.endswith("_lumos"):
                ...
                _transforms.RepackTransform(
                    {
                        "left_view": lk,
                        "right_view": rk,
                        "third_view": self.third_image_key,
                        "left_eef_pos": "observation.state.left_eef_pos",
                        "left_eef_rotvec": "observation.state.left_eef_rotvec",
                        "left_gripper": "observation.state.left_gripper",
                        "right_eef_pos": "observation.state.right_eef_pos",
                        "right_eef_rotvec": "observation.state.right_eef_rotvec",
                        "right_gripper": "observation.state.right_gripper",
                        "demo_start_pose_left": "observation.state.demo_start_pose_left",
                        "demo_start_pose_right": "observation.state.demo_start_pose_right",
                        "left_action": "left_action",
                        "right_action": "right_action",
                        "task": "task",
                    }
        ...
        InputsCls = _dual_franka_inputs_class(self.variant)
        OutputsCls = _dual_franka_outputs_class(self.variant)
        data_transforms = _transforms.Group(
            inputs=[
                InputsCls(
                    model_type=model_config.model_type,
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotXVDualDataConfig(
            # repo_id="/home/ubuntu/qiuyi/ckpts/ckpt_xv_0122_s1/10000/assets",
            # repo_id="local/umi_stock_shelves_0128",
            repo_id="/mnt/public1/chenshuaiwen/.cache/hf_home/fastumi/handover_umi",
            base_config=DataConfig(
                prompt_from_task=True,  # 用 dataset 的 "task" 字段做 prompt
            ),
            assets=AssetsConfig(
                assets_dir="/mnt/public1/chenshuaiwen/.cache/hf_home/fastumi",
                # asset_id="handover_umi",
            ),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader(
            "/mnt/public1/chenshuaiwen/.cache/openpi/openpi-assets/checkpoints/pi05_base/params"
        ),
        lr_schedule=_optimizer.CosineDecaySchedule(
                warmup_steps=1_000,
                peak_lr=5e-5,
        ...
        batch_size=128,
        checkpoint_base_dir="/mnt/public1/chenshuaiwen/checkpoints",
    ),

    TrainConfig(
        name="pi05_dual_franka_finetune_high_lumos",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
            action_horizon=10,
        ...
        data=LeRobotDualFrankaDataConfig(
            repo_id="/mnt/public1/chenshuaiwen/.cache/hf_home/dual_franka/handover_mix_8sets",
            variant="high_lumos",
            assets=AssetsConfig(
                    assets_dir="/mnt/public1/chenshuaiwen/.cache/hf_home/dual_franka",
                    asset_id="handover_mix_8sets",
                ),
            base_config=DataConfig(
                prompt_from_task=True,
    ...
    TrainConfig(
        name="pi05_dual_franka_finetune_high_rs",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
            action_horizon=10,
    ...
    TrainConfig(
        name="pi05_dual_franka_finetune_mask_lumos",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
            action_horizon=10,
    ...
    TrainConfig(
        name="pi05_dual_franka_finetune_mask_rs",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
            action_horizon=10,
    ...
    TrainConfig(
        ...
        data=LeRobotDualFrankaDataConfig(
            repo_id="/mnt/public1/chenshuaiwen/.cache/hf_home/dual_franka/handover_high_absdelta",
    ...
    TrainConfig(
        ...
        data=LeRobotXV13DataConfig(
            repo_id="/mnt/public1/chenshuaiwen/.cache/hf_home/fastumi/batch_pick_up_cubes_0127",
            # repo_id="/home/ubuntu/qiuyi/ckpts/ckpt_xv_0116/20000/assets",
            base_config=DataConfig(
                prompt_from_task=True,  # 用 dataset 的 "task" 字段做 prompt
            ),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("/mnt/public1/chenshuaiwen/.cache/openpi/openpi-assets/checkpoints/pi05_base/params"),
        lr_schedule=_optimizer.CosineDecaySchedule(
            warmup_steps=1_000,
            peak_lr=5e-5,
            decay_steps=1_000_000,
