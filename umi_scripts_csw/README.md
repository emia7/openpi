...
**用途**：采集中 **a/c** 播**两种 chirp**，后期**纯本地**检峰 + 切片。
...
长录切分请用 **`nano_sync_freq/`** 与 [FREQ_WORKFLOW.md](nano_sync_freq/FREQ_WORKFLOW.md)。原 **`nano_sync/`**（TTS 口语锚点 + 百炼 filetrans 切分）已整目录删除，不再维护。
